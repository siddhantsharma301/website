...
const Collection = dynamic(
  async () => (await import('react-notion-x/build/third-party/collection')).Collection,
);
const Equation = dynamic(
  async () => (await import('react-notion-x/build/third-party/equation')).Equation,
)
const Pdf = dynamic(
  async () => (await import('react-notion-x/build/third-party/pdf')).Pdf,
  {
    ssr: false
  }
)
const Modal = dynamic(
  async () => (await import('react-notion-x/build/third-party/modal')).Modal,
  {
    ssr: false
  }
)
...
const Blog: NextPage<BlogProps> = ({ page, recordMap }) => (
  <Container width={['100%', 1200]} maxWidth="100vw">
    ...
    <Container textAlign="center" gridGap=".4rem" my="3rem">
      <Text margin={0}>By Siddhant Sharma</Text>
